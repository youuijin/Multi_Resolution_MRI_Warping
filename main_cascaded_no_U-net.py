...
def train_model(image_paths, template_path, loss='MSE', reg='TV', epochs=200, batch_size=1, lr=1e-4, alpha=0.5, res_level=3, val_interval=5, val_detail=False, start_epoch=0, saved_path=None):
    ...
    train_loader, val_loader, save_loader = set_dataloader(image_paths, template_path, batch_size, feature_load=True, pre_loss=prev_loss)
    ...
    for epoch in range(epochs):
        ...
        for (img, template, _, _, _, dis_fields, x2_features) in tqdm(train_loader, desc=f"Epoch {epoch}"):
            img, template = img.unsqueeze(1).cuda(), template.unsqueeze(1).cuda()

            dis_fields = [d.cuda() for d in dis_fields]
            x2_features = [f.cuda() for f in x2_features]
        ...
        print(f"Epoch {epoch}/{epochs}, Train Loss: {fus_tot_loss / len(train_loader)}")
        wandb.log({
        ...
        if epoch%val_interval == 0:
            ...
            with torch.no_grad():
                for (img, template, _, _, _, dis_fields, x2_features) in val_loader:
                    img, template = img.unsqueeze(1).cuda(), template.unsqueeze(1).cuda()
                    dis_fields = [d.cuda() for d in dis_fields]
                    x2_features = [f.cuda() for f in x2_features]
                    ...
                    for i in range(len(res_k)):
                        weights_mean[i] += [weights[:, i, ...].mean().cpu().numpy()]

                if val_detail:
                    avg_dice, _ = test(prev_loss, fus_model, res_k=res_k)
                    wandb.log({
        ...
        if epoch%15 == 0:
            fus_model.eval()
            with torch.no_grad():
                for i, (img, template, img_min, img_max, affine, dis_fields, x2_features) in enumerate(save_loader):
                    if i>1:
                        break
                    img, template = img.unsqueeze(1).cuda(), template.unsqueeze(1).cuda()
                    dis_fields = [d.cuda() for d in dis_fields]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--image_path", type=str, default="data/brain_core_zero_np")
